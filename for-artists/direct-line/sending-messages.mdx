---
title: "Send a Direct Line message"
description: "Send a message to every fan with one tap. Every post sends them a push notification."
sidebarTitle: "Send a message"
---

Direct Line is the highest-reach tool on your page. Every post sends a push notification to every member's phone. No algorithm, no feed, no guessing who saw it. Write like you would a voice note or a caption, not a long post.

You can send text, voice notes, photos, videos, and audio files. Each post still sends one push notification, no matter how much is in it. To lock a post so only paying subscribers can open it, see [Post subscriber-only content](/for-artists/direct-line/post-subscriber-only-content).

## Send a text message

Open Direct Line from the bottom nav. The paper-plane tab.

![Empty Direct Line view](screenshots/directline-empty.png)

1. Open Direct Line from the bottom nav.
2. Tap the message input at the bottom.
3. Type your message.
4. Tap the send arrow.

Your message shows in the feed with your avatar, your artist name, a timestamp, and a three-dot menu for editing. The newest messages are at the bottom.

## Record a voice message

Tap the microphone icon in the compose bar to start recording.

![Voice message recording in progress](screenshots/directline-voice-message-recording.png)

Stop recording when you're done. A review screen lets you play it back before sending.

![Voice recording review](screenshots/directline-voice-recording-overview.png)

Tap the green checkmark to send, or the red trash icon to discard and try again.

## Attach photos, videos, or audio

Tap the **+** button in the compose bar. It's to the left of the message input.

![Attachment options expanded](screenshots/directline-attachment-options.png)

Three options appear:

- **Camera.** Take a new photo.
- **Media.** Pick photos or videos from your library. You can select more than one at once.
- **Audio.** Upload an audio file from your phone.

You can add a caption with any of these. Every post still sends one push notification.

### Send multiple photos

Pick more than one photo from your library and they stack in the composer as a row with a count badge.

![Composing with multiple photos](screenshots/directline-composer-multi-photo.png)

Add a caption if you want, then send. The photos show up in the feed as a grid.

### Send a video

Pick a video from your library. It shows in the feed with a play-button overlay. Captions work the same as photo posts.

![Video message in feed](screenshots/directline-video-message.png)

## Delete a message

Tap the three-dot menu on any message you've sent and choose **Delete Message**.

![Delete message option](screenshots/directline-delete-message.png)

## The push notification

Every Direct Line post sends a push notification to every member (or to subscribers only, if you used subscriber-only mode).

![Push notification banner](screenshots/directline-push-notification.png)

## Related

- [Post subscriber-only content](/for-artists/direct-line/post-subscriber-only-content)
- [Run your community chat](/for-artists/chat/community-chat)
- [Share your Kollekt link](/for-artists/sharing/sharing-your-page)
