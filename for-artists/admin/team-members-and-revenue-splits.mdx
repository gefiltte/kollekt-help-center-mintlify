---
title: "Add team members and split revenue"
description: "Share the money with your team like your manager, label, or other artists. Only the owner can change the split."
sidebarTitle: "Team and splits"
---

If you work with a manager, label, or featured artist, revenue splits mean everyone gets paid automatically. No spreadsheets. Pick a percentage on the Payments screen and they take that share of every sale. Stripe handles the rest.

Open Payments from the Settings gear in the bottom nav, then tap **Edit** to go to your editor and find **Payments**.

## What you see

You'll see two sections:

- **Earnings.** Money from all sales. Split into **Pending** (waiting for payout) and **Paid** (already sent).
- **Default split.** How money divides across the team. Tap the info icon on either section for more.

![Payments screen with the default split being edited](screenshots/admin-payments-split-editing.png)

## Add someone to the split

The total can't go over 100%. Whatever's left stays with you.

1. Tap **Add user to split**.
2. Search for the team member by name or username.
3. Type their percentage.
4. Tap the checkmark to confirm.

![Typing the split percentage](screenshots/admin-payments-add-user-percent.png)

The row shows **Shared [percentage]%** at the bottom. Tap **Save** to lock it in.

![Split added confirmation](screenshots/admin-payments-split-added.png)

A green **Split added** banner confirms.

## Remove someone from the split

Swipe left on the team member's row, or tap the delete icon. **Remove** appears. The percentage goes back to 100% for you (or splits across other team members if there are any).

## Related

- [Connect your payout account](/for-artists/subscriptions/connect-your-payout-account)
- [How subscriptions and earnings work](/for-artists/subscriptions/how-subscriptions-and-earnings-work)
- [Manage admins and roles](/for-artists/admin/manage-admins-and-roles)
